...
// ---------------------------------------------------------------------------
// Types — matches /api/platforms/status response
// ---------------------------------------------------------------------------
...
// Derive a display status from the raw API fields
function getDisplayStatus(p: PlatformStatusData): 'synced' | 'syncing' | 'error' | 'pending' | 'idle' {
  if (p.syncStatus === 'IN_PROGRESS') return 'syncing';
  if (p.lastSyncError || p.consecutiveFailures >= 3) return 'error';
  if (p.connectionStatus === 'connected' && p.syncStatus === 'SUCCESS') return 'synced';
  if (p.connectionStatus === 'connected') return 'idle';
  return 'pending';
}
...
export function SyncStatusWidget({ className }: SyncStatusWidgetProps) {
  ...
  const syncingCount = platforms.filter(p => p.syncStatus === 'IN_PROGRESS').length;
  const errorCount = platforms.filter(p => p.lastSyncError || p.consecutiveFailures >= 3).length;
  ...
  return (
    ...
    >
      <div className="glass-card h-full p-6 relative overflow-hidden flex flex-col transition-all duration-300">
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          ...
          <Button
            ...
            disabled={syncing}
            className="hover:bg-zinc-100 dark:hover:bg-white/5 h-8 px-3"
          >
            {syncing ? (
        ...
        <div className="flex-1 space-y-2 overflow-y-auto">
          {platforms.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-8">
              <WifiOff className="w-10 h-10 text-zinc-300 dark:text-zinc-700 mb-3" />
              <p className="text-zinc-900 dark:text-white font-bold">No Platforms Connected</p>
              <p className="text-zinc-500 dark:text-zinc-500 text-sm mt-1">Connect platforms to sync your progress</p>
            ...
            <AnimatePresence mode="popLayout">
              {platforms.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    {/* Platform Icon */}
                    <div className="w-8 h-8 rounded-lg bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-700 flex items-center justify-center shrink-0">
                      {p.icon ? (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] font-bold text-zinc-500">
                        {item.lastSyncedAt
                          ? `Last: ${formatDistanceToNow(new Date(item.lastSyncedAt), { addSuffix: true })}`
                          : 'Never synced'}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <StatusIcon className={cn(
                        "w-4 h-4",
                        cfg.color,
                        displayStatus === 'syncing' && "animate-spin"
                      )} />
        ...
        {platforms.length > 0 && (
          <div className="mt-4 pt-4 border-t border-zinc-200 dark:border-zinc-800">
            ...
            >
              View All Platforms <ChevronRight className="w-3 h-3 group-hover:translate-x-1 transition-transform" />
            </Link>
          </div>
        )}
      </div>
