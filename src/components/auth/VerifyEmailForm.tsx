 ...
 * Location: components/auth/VerifyEmailForm.tsx
 * 
...
// Interface for EmailVerification model
interface IEmailVerification {
  id: string;
  // Add fields from your Prisma EmailVerification model
  // Check schema.prisma for exact field definitions
  createdAt: Date;
  updatedAt: Date;
}
...
// Example API calls:
// /api/auth/verify-email
const fetchVerifyEmailFormData = async () => {
  try {
...
const fetchVerifyEmailFormData = async () => {
  try {
    const response = await apiClient.get('/api/auth/resend-verification');
    return response.data;
  } catch (error) {
    console.error('Error fetching data:', error);
    throw error;
