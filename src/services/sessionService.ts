...
export class SessionService {
  ...
  static async createSession(
    ...
    try {
      await cache.del(`session:count:${userId}`);
    } catch (e) {
      // limit impact of cache errors
    }
  ...
  /**
   * Get active sessions count for a user (Cached)
   */
  static async getCachedSessionCount(userId: string): Promise<number> {
    const cacheKey = `session:count:${userId}`;
    ...
    try {
      ...
      logger.error('Error getting cached session count', { userId }, error);
    }
  ...
  /**
   * Get active sessions count for a user (Direct DB)
   */
  ...
  /**
   * Invalidate session count cache helper
   */
  private static async invalidateSessionCountCache(userId: string) {
    try {
      await cache.del(`session:count:${userId}`);
    } catch (e) {
      // ignore
  ...
  static async revokeSessionByToken(
    ...
    if (session) {
      await this.invalidateSessionCountCache(session.userId);
    }
  ...
  static async revokeAllSessions(
    ...
    await this.invalidateSessionCountCache(userId);
