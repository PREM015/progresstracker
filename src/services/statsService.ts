...
export class StatsService {
  ...
  static async calculateStreak(userId: string): Promise<StreakData> {
    // Load user for longestStreak comparison only — do NOT return cached values
    const user = await prisma.user.findUnique({
      where: { id: userId },
      select: { longestStreak: true },
    });

    // ALWAYS recalculate from TrackerEntry data
  ...
  /**
   * Recalculate and sync all user profile totals from connected platforms
   * This ensures that counts from GitHub, LeetCode, etc. are reflected in the main profile
   */
  static async recalculateUserStats(userId: string): Promise<any> {
    const userPlatforms = await prisma.userPlatform.findMany({
      where: { userId, isActive: true },
      select: { cachedStats: true, platformId: true }
    });
    ...
    userPlatforms.forEach(up => {
      const stats = up.cachedStats as any;
      if (!stats) return;

      // Aggregate from metadata/cachedStats
      totalProblems += (stats.totalProblems || stats.solvedCount || stats.problemsSolved || 0);
      totalCommits += (stats.totalCommits || stats.commits || 0);
      totalPRs += (stats.totalPRs || stats.pullRequests || 0);
      totalTimeSpent += (stats.totalTimeSpent || stats.timeSpent || 0);
      totalPoints += (stats.totalPoints || stats.points || 0);
    });

    // Also factor in manual/synced TrackerEntry records to be safe
    ...
    // Use the max of scrapers vs entries to ensure we don't lose data
    const finalStats = {
      totalProblems: Math.max(totalProblems, entryStats._sum.problemsSolved || 0),
      totalCommits: Math.max(totalCommits, entryStats._sum.commits || 0),
      totalPullRequests: Math.max(totalPRs, entryStats._sum.pullRequests || 0),
      totalTimeSpent: Math.max(totalTimeSpent, entryStats._sum.timeSpent || 0),
      totalPoints: Math.max(totalPoints, entryStats._sum.points || 0),
    };
   ...
   * Optimized: uses DailyStats table
   */
  static async getHeatmapData(userId: string, options: { days?: number, year?: number, timezone?: string } = {}): Promise<HeatmapDataPoint[]> {
    const { days, year } = options;
    const cacheKey = `stats:heatmap:${userId}:${year || days || 'default'}`;
    const cached = await CacheService.get(cacheKey);
    ...
    if (year) {
      ...
      startDate = startOfDay(subDays(new Date(), dayCount));
  ...
  static async getPlatformStats(userId: string): Promise<{
    platforms: Array<{
      platform: { id: string; name: string; slug: string; category: string; icon: string | null; color: string | null };
      stats: {
        totalProblems: number;
        totalCommits: number;
        totalTime: number;
