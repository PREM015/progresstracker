...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const rateLimitKey = `email-webhook-open:${ip}`;
    ...
    // Record email open in EmailLog
    const emailLog = await prisma.emailLog.updateMany({
      where: { email: data.email, messageId: data.messageId } as any,
      data: { status: 'OPENED', openedAt: new Date(data.ts * 1000) },
    });

    const result = { processed: emailLog.count, email: data.email };

    logger.info('Email open webhook processed', {
      requestId,
      email: data.email,
    ...
    logger.error('Email open webhook failed', { requestId }, error);
