...
// Tags: cron, platform, health
// Generated: 2026-02-02T11:57:44.591415
// =============================================================================
...
async function validateAdminSession(request: NextRequest, requestId: string) {
  ...
  const rateLimitKey = `cron-platform-health:${ip}`;
...
/**
 * POST - Check platform health
 * 
 * TODO Implementation Checklist:
   * - Validate cron secret or admin session
   * - Get all active platforms
   * - Perform health check for each platform
   * - Test authentication endpoints
   * - Test scraping if applicable
   * - Update platform health status
   * - Alert admins for failing platforms
...
export async function POST(
  ...
  try {
    ...
    logger.info('POST cron/platform-health completed', {
      userId,
      requestId,
    ...
    logger.error('POST cron/platform-health failed', { requestId }, error);
