import { NextRequest, NextResponse } from 'next/server';
import { getServerSession } from 'next-auth';
import { authOptions } from '@/lib/auth';
import { logger } from '@/lib/logger';
import { StatsService } from '@/services/statsService';
import { subDays, startOfDay, endOfDay } from 'date-fns';

export async function GET(req: NextRequest) {
  try {
    const session = await getServerSession(authOptions);
    if (!session?.user?.id) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    const { searchParams } = new URL(req.url);
    const days = parseInt(searchParams.get('days') || '30');
    const metric = (searchParams.get('metric') || 'problems') as 'problems' | 'time' | 'commits';

    const startDate = startOfDay(subDays(new Date(), days));
    const endDate = endOfDay(new Date());

    const trends = await StatsService.getTrendData(
      session.user.id,
      startDate,
      endDate,
      metric
    );

    // Calculate growth rate
    const midpoint = Math.floor(trends.length / 2);
    const firstHalf = trends.slice(0, midpoint);
    const secondHalf = trends.slice(midpoint);

    const firstHalfAvg = firstHalf.length > 0
      ? firstHalf.reduce((sum, item) => sum + item.value, 0) / firstHalf.length
      : 0;
    
    const secondHalfAvg = secondHalf.length > 0
      ? secondHalf.reduce((sum, item) => sum + item.value, 0) / secondHalf.length
      : 0;

    const growthRate = firstHalfAvg > 0
      ? ((secondHalfAvg - firstHalfAvg) / firstHalfAvg) * 100
      : 0;

    const total = trends.reduce((sum, item) => sum + item.value, 0);
    const average = trends.length > 0 ? total / trends.length : 0;
    const peak = trends.length > 0 ? Math.max(...trends.map(item => item.value)) : 0;
    const activeDays = trends.filter(item => item.value > 0).length;

    return NextResponse.json({
      trends,
      metrics: {
        total: Math.round(total),
        average: Math.round(average * 10) / 10,
        growthRate: Math.round(growthRate * 10) / 10,
        peak,
        activeDays,
      }
    });
  } catch (error) {
    console.error('Trends API error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch trend data' },
      { status: 500 }
    );
  }
}