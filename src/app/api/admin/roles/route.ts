...
// Generated: 2026-02-02T11:57:44.551360
// =============================================================================
...
async function validateAdminSession(request: NextRequest, requestId: string) {
  ...
  const rateLimitKey = `admin-roles:${ip}`;
 ...
 * TODO Implementation Checklist:
   ...
   * - DELETE: Delete role (check no users assigned)
...
export async function GET(
  ...
  try {
    ...
    logger.info('GET admin/roles completed', {
      userId,
      page,
      total,
      requestId,
    ...
    logger.error('GET admin/roles failed', { requestId }, error);
...
export async function POST(
  ...
  try {
    ...
    logger.info('POST admin/roles completed', {
      userId,
      requestId,
      duration: Date.now() - startTime,
    });
    ...
    logger.error('POST admin/roles failed', { requestId }, error);
    return addHeaders(apiResponse.internalError('Operation failed', requestId), requestId);
  }
}
...
export async function PUT(
  ...
  try {
    ...
    logger.info('PUT admin/roles completed', {
      userId,
      requestId,
      duration: Date.now() - startTime,
    });
    ...
    logger.error('PUT admin/roles failed', { requestId }, error);
    return addHeaders(apiResponse.internalError('Operation failed', requestId), requestId);
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    logger.info('DELETE admin/roles completed', {
      userId,
      requestId,
      duration: Date.now() - startTime,
    });
    ...
    logger.error('DELETE admin/roles failed', { requestId }, error);
    return addHeaders(apiResponse.internalError('Operation failed', requestId), requestId);
  }
}
