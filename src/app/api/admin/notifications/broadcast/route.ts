...
import { notificationTask } from "@/trigger/notification-task";
...
export const POST = withErrorHandling(async (req: NextRequest) => {
  ...
  if (channels.includes('IN_APP')) {
    // Create notifications via Trigger.dev
    ...
    // Trigger one task per batch instead of addBulk
    promises.push(notificationTask.trigger({
  ...
  if (channels.includes('EMAIL')) {
    ...
    if (emailRecipients.length > 0) {
      promises.push(emailTask.trigger({
