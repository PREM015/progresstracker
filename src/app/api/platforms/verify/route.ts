...
const log = logger.child({ route: 'platforms/verify' });
 ...
 */
function buildScraperCredentials(connection: ConnectionData): ScraperCredentials {
...
async function testPlatformConnection(
  ...
  try {
    ...
    if (!scraper) {
      // Platform doesn't have a scraper - assume valid if credentials exist
    ...
    log.error('Platform connection test failed', { platformSlug }, error);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (connection.isVerified && !forceRevalidate) {
      ...
      log.info('Platform already verified', {
        userId,
        platformId,
      ...
      return addHeaders(
        apiResponse.success(result, { meta: { requestId, cached: true } }),
        requestId,
        rateLimitResult
      );
    }
    ...
    // Test the connection
    const testResult = await testPlatformConnection(connection.platform.slug, credentials);
    ...
    if (!testResult.success) {
      ...
      log.warn('Platform verification failed', {
        userId,
        platformId,
        platformSlug: connection.platform.slug,
    ...
    if (connection.autoSync && connection.platform.supportsAutoSync) {
      await PlatformService.scheduleNextSync(userId, platformId, 1);

      log.info('Scheduled initial sync after verification', {
        userId,
        platformId,
        platformSlug: connection.platform.slug,
      });
    ...
    log.info('Platform verified successfully', {
      userId,
      platformId,
      platformSlug: connection.platform.slug,
      requestId,
    ...
    log.error('POST /api/platforms/verify failed', { requestId }, error);
