...
function calculatePeriodDates(type: string, period?: any, customPeriod?: any) {
  const now = new Date();
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const { type, period, customPeriod, config, priority, async: isAsync } = validation.data;

    // Calculate period dates
    const { start, end } = calculatePeriodDates(type, period, customPeriod);
