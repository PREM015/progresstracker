...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
    ...
    try {
        ...
        const rateLimitResult = await checkLimit(apiRateLimiter, RATE_LIMIT, `stripe:invoices:download:${session.user.id}`);
        ...
        if (!invoice.invoice_pdf) {
            return addHeaders(apiResponse.notFound('PDF not available', requestId), requestId, rateLimitResult);
        }
        ...
        logger.error('GET stripe invoice download failed', { requestId }, error);
