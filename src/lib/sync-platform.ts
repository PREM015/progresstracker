...
export async function syncPlatformData(
  ...
  try {
    ...
    logger.info('Fetching data from platform', { 
    ...
    await prisma.userPlatform.update({
      where: { id: userPlatformId },
      data: {
        cachedStats: normalizedData as object,
 ...
 */
function extractCredentials(userPlatform: {
  ...
  return {
    ...
    accessToken: userPlatform.accessToken || undefined,
    refreshToken: userPlatform.refreshToken || undefined,
    credentials: userPlatform.credentials as Record<string, unknown> | undefined,
  };
}
...
async function updateUserPlatformStats(
  ...
  try {
    await prisma.userPlatform.update({
      where: { id: userPlatformId },
      data: {
        platformData: data as object,
        cachedStats: data as object,
        statsUpdatedAt: new Date(),
      },
    });
  } catch (error) {
